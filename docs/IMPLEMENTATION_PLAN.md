...
- `GET https://auth.celeris.pro/login?loginRequestId=...`
...
| [WO-06](./WORK_ORDERS/WO-06-developer-api-and-server-sdk.md) | Developer API hardening and server SDK | protected developer API and backend SDK | WO-01, WO-03, WO-04, WO-05.5, WO-05.6 |
| [WO-07](./WORK_ORDERS/WO-07-legacy-removal-and-regression.md) | Legacy removal and regression rewrite | deletion of incompatible files/routes/tests and final green suite | WO-01, WO-02, WO-03, WO-04, WO-05, WO-05.5, WO-05.6, WO-06 |
...
The plan is intentionally split into one foundation cut, two backend player slices, one action/custody cut, one player demo slice, one auth-ownership pivot, one hosted-auth slice, one developer integration slice, and one final deletion/regression WO.
...
- `WO-06` moves the developer surface to a real protected API and server SDK only after the auth-ownership and hosted-login contracts are stable.
...
- all nine work orders are complete
- `npm test` passes
